...
describe("Logger", () => {
  ...
  describe("error stacks", () => {
    ...
    it("logs stack if given", () => {
      ...
      log.error(err);
      transport.spies.error.should.be.calledWith(`(ThisTest) [ERROR] ${err.message}`);
    });
  ...
  describe("error keys", () => {
    ...
    it("logs key if given", () => {
      const keyMessage = { key: 'KEY', message: 'MESSAGE' };
      log.error(keyMessage);
      transport.spies.error.should.be.calledWith(`(ThisTest) [ERROR] ${keyMessage.key}: ${keyMessage.message}`);
    });
